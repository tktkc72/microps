...
ip_iface_alloc(const char *unicast, const char *netmask)
{
    ...
    {
        errorf("convert unicast failure. unicast=%s", unicast);
        return NULL;
    }
    if (ip_addr_pton(netmask, &iface->netmask) == -1)
    {
        errorf("convert netmask failure. netmask=%s", netmask);
        return NULL;
    }
...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if (len < total)
    {
        errorf("too short len: %u, total: %u", len, hdr->total);
        return;
    }
    ...
    if (!iface)
    {
        errorf("not found ip_iface, addr=%s", dev->addr);
    }
...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    else
    {
        ...
        if (!iface)
        {
            errorf("not found ip_iface addr=%s", ip_addr_ntop(src, addr, sizeof(addr)));
            return -1;
        }
        if ((dst & iface->netmask) != (iface->unicast & iface->netmask) && dst != IP_ADDR_BROADCAST)
        {
            errorf("not reached, addr=%s", ip_addr_ntop(dst, addr, sizeof(addr)));
            return -1;
        }
    }
